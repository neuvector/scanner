// Code generated by protoc-gen-go. DO NOT EDIT.
// source: common.proto

package share

import proto "github.com/golang/protobuf/proto"
import fmt "fmt"
import math "math"

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion2 // please upgrade the proto package

type ProfilingCmd int32

const (
	ProfilingCmd_StartProfiling ProfilingCmd = 0
	ProfilingCmd_StopProfiling  ProfilingCmd = 1
)

var ProfilingCmd_name = map[int32]string{
	0: "StartProfiling",
	1: "StopProfiling",
}
var ProfilingCmd_value = map[string]int32{
	"StartProfiling": 0,
	"StopProfiling":  1,
}

func (x ProfilingCmd) String() string {
	return proto.EnumName(ProfilingCmd_name, int32(x))
}
func (ProfilingCmd) EnumDescriptor() ([]byte, []int) {
	return fileDescriptor_common_74d6bcd5429cd06b, []int{0}
}

type ProfilingMethod int32

const (
	ProfilingMethod_CPU    ProfilingMethod = 0
	ProfilingMethod_Memory ProfilingMethod = 1
)

var ProfilingMethod_name = map[int32]string{
	0: "CPU",
	1: "Memory",
}
var ProfilingMethod_value = map[string]int32{
	"CPU":    0,
	"Memory": 1,
}

func (x ProfilingMethod) String() string {
	return proto.EnumName(ProfilingMethod_name, int32(x))
}
func (ProfilingMethod) EnumDescriptor() ([]byte, []int) {
	return fileDescriptor_common_74d6bcd5429cd06b, []int{1}
}

type RPCVoid struct {
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *RPCVoid) Reset()         { *m = RPCVoid{} }
func (m *RPCVoid) String() string { return proto.CompactTextString(m) }
func (*RPCVoid) ProtoMessage()    {}
func (*RPCVoid) Descriptor() ([]byte, []int) {
	return fileDescriptor_common_74d6bcd5429cd06b, []int{0}
}
func (m *RPCVoid) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_RPCVoid.Unmarshal(m, b)
}
func (m *RPCVoid) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_RPCVoid.Marshal(b, m, deterministic)
}
func (dst *RPCVoid) XXX_Merge(src proto.Message) {
	xxx_messageInfo_RPCVoid.Merge(dst, src)
}
func (m *RPCVoid) XXX_Size() int {
	return xxx_messageInfo_RPCVoid.Size(m)
}
func (m *RPCVoid) XXX_DiscardUnknown() {
	xxx_messageInfo_RPCVoid.DiscardUnknown(m)
}

var xxx_messageInfo_RPCVoid proto.InternalMessageInfo

type CLUSProfilingRequest struct {
	Cmd                  ProfilingCmd      `protobuf:"varint,1,opt,name=Cmd,proto3,enum=share.ProfilingCmd" json:"Cmd,omitempty"`
	Methods              []ProfilingMethod `protobuf:"varint,2,rep,packed,name=Methods,proto3,enum=share.ProfilingMethod" json:"Methods,omitempty"`
	Duration             uint32            `protobuf:"varint,3,opt,name=Duration,proto3" json:"Duration,omitempty"`
	XXX_NoUnkeyedLiteral struct{}          `json:"-"`
	XXX_unrecognized     []byte            `json:"-"`
	XXX_sizecache        int32             `json:"-"`
}

func (m *CLUSProfilingRequest) Reset()         { *m = CLUSProfilingRequest{} }
func (m *CLUSProfilingRequest) String() string { return proto.CompactTextString(m) }
func (*CLUSProfilingRequest) ProtoMessage()    {}
func (*CLUSProfilingRequest) Descriptor() ([]byte, []int) {
	return fileDescriptor_common_74d6bcd5429cd06b, []int{1}
}
func (m *CLUSProfilingRequest) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_CLUSProfilingRequest.Unmarshal(m, b)
}
func (m *CLUSProfilingRequest) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_CLUSProfilingRequest.Marshal(b, m, deterministic)
}
func (dst *CLUSProfilingRequest) XXX_Merge(src proto.Message) {
	xxx_messageInfo_CLUSProfilingRequest.Merge(dst, src)
}
func (m *CLUSProfilingRequest) XXX_Size() int {
	return xxx_messageInfo_CLUSProfilingRequest.Size(m)
}
func (m *CLUSProfilingRequest) XXX_DiscardUnknown() {
	xxx_messageInfo_CLUSProfilingRequest.DiscardUnknown(m)
}

var xxx_messageInfo_CLUSProfilingRequest proto.InternalMessageInfo

func (m *CLUSProfilingRequest) GetCmd() ProfilingCmd {
	if m != nil {
		return m.Cmd
	}
	return ProfilingCmd_StartProfiling
}

func (m *CLUSProfilingRequest) GetMethods() []ProfilingMethod {
	if m != nil {
		return m.Methods
	}
	return nil
}

func (m *CLUSProfilingRequest) GetDuration() uint32 {
	if m != nil {
		return m.Duration
	}
	return 0
}

type CLUSBoolean struct {
	Value                bool     `protobuf:"varint,1,opt,name=Value,proto3" json:"Value,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *CLUSBoolean) Reset()         { *m = CLUSBoolean{} }
func (m *CLUSBoolean) String() string { return proto.CompactTextString(m) }
func (*CLUSBoolean) ProtoMessage()    {}
func (*CLUSBoolean) Descriptor() ([]byte, []int) {
	return fileDescriptor_common_74d6bcd5429cd06b, []int{2}
}
func (m *CLUSBoolean) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_CLUSBoolean.Unmarshal(m, b)
}
func (m *CLUSBoolean) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_CLUSBoolean.Marshal(b, m, deterministic)
}
func (dst *CLUSBoolean) XXX_Merge(src proto.Message) {
	xxx_messageInfo_CLUSBoolean.Merge(dst, src)
}
func (m *CLUSBoolean) XXX_Size() int {
	return xxx_messageInfo_CLUSBoolean.Size(m)
}
func (m *CLUSBoolean) XXX_DiscardUnknown() {
	xxx_messageInfo_CLUSBoolean.DiscardUnknown(m)
}

var xxx_messageInfo_CLUSBoolean proto.InternalMessageInfo

func (m *CLUSBoolean) GetValue() bool {
	if m != nil {
		return m.Value
	}
	return false
}

type CLUSMetry struct {
	CPU                  float64  `protobuf:"fixed64,1,opt,name=CPU,proto3" json:"CPU,omitempty"`
	Memory               uint64   `protobuf:"varint,2,opt,name=Memory,proto3" json:"Memory,omitempty"`
	SessionIn            uint32   `protobuf:"varint,3,opt,name=SessionIn,proto3" json:"SessionIn,omitempty"`
	SessionOut           uint32   `protobuf:"varint,4,opt,name=SessionOut,proto3" json:"SessionOut,omitempty"`
	SessionCurIn         uint32   `protobuf:"varint,5,opt,name=SessionCurIn,proto3" json:"SessionCurIn,omitempty"`
	SessionCurOut        uint32   `protobuf:"varint,6,opt,name=SessionCurOut,proto3" json:"SessionCurOut,omitempty"`
	PacketIn             uint64   `protobuf:"varint,7,opt,name=PacketIn,proto3" json:"PacketIn,omitempty"`
	PacketOut            uint64   `protobuf:"varint,8,opt,name=PacketOut,proto3" json:"PacketOut,omitempty"`
	ByteIn               uint64   `protobuf:"varint,9,opt,name=ByteIn,proto3" json:"ByteIn,omitempty"`
	ByteOut              uint64   `protobuf:"varint,10,opt,name=ByteOut,proto3" json:"ByteOut,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *CLUSMetry) Reset()         { *m = CLUSMetry{} }
func (m *CLUSMetry) String() string { return proto.CompactTextString(m) }
func (*CLUSMetry) ProtoMessage()    {}
func (*CLUSMetry) Descriptor() ([]byte, []int) {
	return fileDescriptor_common_74d6bcd5429cd06b, []int{3}
}
func (m *CLUSMetry) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_CLUSMetry.Unmarshal(m, b)
}
func (m *CLUSMetry) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_CLUSMetry.Marshal(b, m, deterministic)
}
func (dst *CLUSMetry) XXX_Merge(src proto.Message) {
	xxx_messageInfo_CLUSMetry.Merge(dst, src)
}
func (m *CLUSMetry) XXX_Size() int {
	return xxx_messageInfo_CLUSMetry.Size(m)
}
func (m *CLUSMetry) XXX_DiscardUnknown() {
	xxx_messageInfo_CLUSMetry.DiscardUnknown(m)
}

var xxx_messageInfo_CLUSMetry proto.InternalMessageInfo

func (m *CLUSMetry) GetCPU() float64 {
	if m != nil {
		return m.CPU
	}
	return 0
}

func (m *CLUSMetry) GetMemory() uint64 {
	if m != nil {
		return m.Memory
	}
	return 0
}

func (m *CLUSMetry) GetSessionIn() uint32 {
	if m != nil {
		return m.SessionIn
	}
	return 0
}

func (m *CLUSMetry) GetSessionOut() uint32 {
	if m != nil {
		return m.SessionOut
	}
	return 0
}

func (m *CLUSMetry) GetSessionCurIn() uint32 {
	if m != nil {
		return m.SessionCurIn
	}
	return 0
}

func (m *CLUSMetry) GetSessionCurOut() uint32 {
	if m != nil {
		return m.SessionCurOut
	}
	return 0
}

func (m *CLUSMetry) GetPacketIn() uint64 {
	if m != nil {
		return m.PacketIn
	}
	return 0
}

func (m *CLUSMetry) GetPacketOut() uint64 {
	if m != nil {
		return m.PacketOut
	}
	return 0
}

func (m *CLUSMetry) GetByteIn() uint64 {
	if m != nil {
		return m.ByteIn
	}
	return 0
}

func (m *CLUSMetry) GetByteOut() uint64 {
	if m != nil {
		return m.ByteOut
	}
	return 0
}

type CLUSStats struct {
	ReadAt               int64      `protobuf:"varint,1,opt,name=ReadAt,proto3" json:"ReadAt,omitempty"`
	Interval             uint32     `protobuf:"varint,2,opt,name=Interval,proto3" json:"Interval,omitempty"`
	Total                *CLUSMetry `protobuf:"bytes,3,opt,name=Total,proto3" json:"Total,omitempty"`
	Span1                *CLUSMetry `protobuf:"bytes,4,opt,name=Span1,proto3" json:"Span1,omitempty"`
	Span12               *CLUSMetry `protobuf:"bytes,5,opt,name=Span12,proto3" json:"Span12,omitempty"`
	Span60               *CLUSMetry `protobuf:"bytes,6,opt,name=Span60,proto3" json:"Span60,omitempty"`
	XXX_NoUnkeyedLiteral struct{}   `json:"-"`
	XXX_unrecognized     []byte     `json:"-"`
	XXX_sizecache        int32      `json:"-"`
}

func (m *CLUSStats) Reset()         { *m = CLUSStats{} }
func (m *CLUSStats) String() string { return proto.CompactTextString(m) }
func (*CLUSStats) ProtoMessage()    {}
func (*CLUSStats) Descriptor() ([]byte, []int) {
	return fileDescriptor_common_74d6bcd5429cd06b, []int{4}
}
func (m *CLUSStats) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_CLUSStats.Unmarshal(m, b)
}
func (m *CLUSStats) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_CLUSStats.Marshal(b, m, deterministic)
}
func (dst *CLUSStats) XXX_Merge(src proto.Message) {
	xxx_messageInfo_CLUSStats.Merge(dst, src)
}
func (m *CLUSStats) XXX_Size() int {
	return xxx_messageInfo_CLUSStats.Size(m)
}
func (m *CLUSStats) XXX_DiscardUnknown() {
	xxx_messageInfo_CLUSStats.DiscardUnknown(m)
}

var xxx_messageInfo_CLUSStats proto.InternalMessageInfo

func (m *CLUSStats) GetReadAt() int64 {
	if m != nil {
		return m.ReadAt
	}
	return 0
}

func (m *CLUSStats) GetInterval() uint32 {
	if m != nil {
		return m.Interval
	}
	return 0
}

func (m *CLUSStats) GetTotal() *CLUSMetry {
	if m != nil {
		return m.Total
	}
	return nil
}

func (m *CLUSStats) GetSpan1() *CLUSMetry {
	if m != nil {
		return m.Span1
	}
	return nil
}

func (m *CLUSStats) GetSpan12() *CLUSMetry {
	if m != nil {
		return m.Span12
	}
	return nil
}

func (m *CLUSStats) GetSpan60() *CLUSMetry {
	if m != nil {
		return m.Span60
	}
	return nil
}

func init() {
	proto.RegisterType((*RPCVoid)(nil), "share.RPCVoid")
	proto.RegisterType((*CLUSProfilingRequest)(nil), "share.CLUSProfilingRequest")
	proto.RegisterType((*CLUSBoolean)(nil), "share.CLUSBoolean")
	proto.RegisterType((*CLUSMetry)(nil), "share.CLUSMetry")
	proto.RegisterType((*CLUSStats)(nil), "share.CLUSStats")
	proto.RegisterEnum("share.ProfilingCmd", ProfilingCmd_name, ProfilingCmd_value)
	proto.RegisterEnum("share.ProfilingMethod", ProfilingMethod_name, ProfilingMethod_value)
}

func init() { proto.RegisterFile("common.proto", fileDescriptor_common_74d6bcd5429cd06b) }

var fileDescriptor_common_74d6bcd5429cd06b = []byte{
	// 455 bytes of a gzipped FileDescriptorProto
	,
}
